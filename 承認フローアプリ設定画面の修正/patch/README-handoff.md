...
| ファイル | 用途 |
|---|---|
| `audit-mock.html` | 修正後デザインの静的モック |
| `audit-design.png` | スクリーンショット |
| `style-append-audit.css` | `style.css` の末尾に追記 |
| `app-showAuditLogScreen.js` | `showAuditLogScreen()` と `renderAuditLogRows()` を丸ごと差し替え＋`clearAuditLogFilter()` / `fetchNameByEmailMap()` を新規追加 |

修正の意図
1. 絞り込みを**1行のバー**に（項目セレクト＋期間＋「条件をクリア」）。従来の縦積み `form-group` をやめた。
2. 行を「時刻 52px｜変更者 96px｜項目 92px｜内容」の**固定列**に。時刻は等幅数字。変更者は**メールアドレスでなく名前**を表示（`fetchNameByEmailMap()` で profiles / notification_recipients からメール一致で引く。見つからなければメールをそのまま、`title` には常にメール）。
3. **日付ごとのグループ見出し**（件数付き）を追加。行には時刻のみ表示して横幅を内容に回す。
4. 項目バッジをカテゴリ別に色分け（名簿編集＝青／通知の宛先＝紫／フローON/OFF＝緑／宛先候補＝黄／会議室＝グレー）。クラスは `audit-cat cat-<category>`。
5. 0件時は枠内に中央寄せのメッセージ。

注意
- クエリ部分（`db.from('settings_audit_log')...`、JST変換）は従来どおり。`AUDIT_CATEGORY_LABELS` / `auditLogFilter` / `applyAuditLogFilter()` も変更なし。

## 4. 担当者の編集画面（名簿管理から開く）
| ファイル | 用途 |
|---|---|
| `member-edit-mock.html` | 修正後デザインの静的モック（ログイン可能／非ログインの2種） |
| `member-edit-design.png` | スクリーンショット |
| `style-append-member-edit.css` | `style.css` の末尾に追記 |
| `app-editRosterMember.js` | `editRosterMember(key)` を丸ごと差し替え |

修正の意図
1. 「ラベル 96px ＋ 入力」の2カラム行（`rm-field`）に統一。
2. 変更できない項目（メール・部署・単一役職）は `disabled` の入力欄をやめ、**読み取り専用の行**（`rm-readonly` ＋ 右端に「変更不可」）で見せる。役職が1択の場合は `rm_tier` を hidden input にして保存処理を従来どおり動かす。
3. 「基本情報」「兼任部署」をカード（`rm-card`）に分割。兼任部署は縦並びチェックから**4列のチップ状チェック**（選択時は青枠）に。
4. 退職処理・削除は赤枠の危険操作カード（`rm-danger`）に隔離し、塗りボタンから枠線ボタンに変更。
5. 保存・キャンセルは本文下部の固定バー（`rm-footer`）に。

注意
- `saveRosterMember()` / `deleteLoginRosterMember()` / `deleteNonLoginRosterMember()` / `onRmDepartmentSelectChange()` は変更不要。
  id（`rm_name` / `rm_email` / `rm_tier` / `rm_active` / `rm_department_select` / `rm_department_other`）とクラス `rm_extra_dept` を維持している。
- 追加画面（`addRosterMember()` / `addLoginRosterMember()`）は今回対象外。同じ `rm-*` クラスで揃えられる。

## 共通の値
- 設定パネル幅 740px 前提。名簿の列幅 `78px 178px 48px 52px minmax(160px,1fr) 52px`、宛先一覧 `minmax(0,1fr) 96px 148px 16px`、宛先詳細 `18px 86px minmax(0,1fr) auto`、変更履歴 `52px 96px 92px minmax(0,1fr)`、いずれも `column-gap: 8〜10px`。
