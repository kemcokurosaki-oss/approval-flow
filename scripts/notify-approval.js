...
async function main() {
  ...
  const requests = reqIds.length > 0 ? await supabaseFetch(
    `approval_requests?id=in.(${reqIds.join(',')})&select=id,project_number,machine_name,unit_name,flow_type,status,note,requester_id,inspection_date,inspection_time,inspection_location,confirmed_shipping_date,is_resubmit,sheet_data`
  ) : [];
