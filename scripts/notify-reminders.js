 ...
 * 6. 試運転準備完了 催促: 試運転タスクの開始予定日の3日前を過ぎても「試運転準備完了」
 *    （電気艤装タスクがあれば組立・電装それぞれ）がチェックされていない場合、
 *    未完了側の担当者へ毎日通知（チェックするまで継続。工番2000番台も対象）
 * 7. 出荷品リスト作成催促: 簡易検査・外観検査・出荷確認会議の案内催促と同じ基準日から、該当工事番号の
 *    設計担当者へ出荷品リストの作成を1回だけ通知（毎日ではない）。開催案内の申請が提出されるまでの
 *    間は未送信の対象者に送られ続けるが、同一対象者には二度送らない。
 *    出荷確認会議がある工事番号では外観検査のタイミングでは送らない（1工事番号で二重送信しないため）。
 *    工番2000番台は案内催促と同様に対象外
 *
...
// 工番5番台・7番台（先頭が5または7）は承認フローアプリで一切管理しないため、表示・通知とも常に対象外にする
const is5or7Series = projectNumber => /^[57]/.test(String(projectNumber).trim());
...
function isFlowExcludedFor2000s(projectNumber, flowType) {
  if (is5or7Series(projectNumber)) return true;
...
async function runSubmissionReminders() {
  ...
  for (const [taskText, flowType] of Object.entries(TASK_TO_FLOW)) {
    ...
    for (const task of dedupedTasks.values()) {
      ...
      if (isSubmissionReminderExcludedFor2000s(task.project_number, flowType)) continue;
...
// ===== 出荷品リスト作成催促 =====
// 簡易検査・外観検査・出荷確認会議の案内催促と同じ基準日から、該当工事番号の設計担当者へ
// 出荷品リストの作成を1回だけ通知する（毎日ではない。approval_notificationsで送信済みを判定）。
// 開催案内の申請が提出された対象は以後スキップされる。
      ...
      for (const unit of units) {
        for (const kind of requiredKinds) {
          ...
          for (const ownerName of ownerNames) {
            ...
            for (const profile of (recipients || [])) {
              await sendReminder(profile, projectNum, testRunLabel, unitLabel, kindLabel);
            }
          }
        }
      }
