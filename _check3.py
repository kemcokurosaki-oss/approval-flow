...
def skip_regular_string(i, quote):
    ...
    while i < n:
        ...
        if s[i] == quote:
            return i + 1
        i += 1
    print('UNTERMINATED STRING starting quote', quote, 'near line', calc_line(i))
    sys.exit(1)
...
def parse_code(i, terminator=None):
    # terminator: None for top-level (EOF), '}template' means: consume until matching '}' for ${...}, then return position AFTER that '}'
    depth = 0  # extra {} depth opened inside ${...} beyond the initial one
    while i < n:
        c = s[i]
        if c in (' ', '\t', '\n', '\r'):
            i += 1
        ...
        if c == '/' and could_be_regex_start(i):
            ...
            if ok:
                ...
                i = j
                continue
            # else fall through as division operator
        if c in '([':
