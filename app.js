...
async function openDetailModal(requestId) {
    ...
    document.getElementById('detail_body').innerHTML = `
        ...
        ${shippingInfoParts.length ? `<div style="font-size:14px;color:#888;margin-top:2px;display:flex;flex-wrap:wrap;column-gap:16px;row-gap:2px;">${shippingInfoParts.map(p => `<span style="white-space:nowrap;">${p}</span>`).join('')}</div>` : ''}
