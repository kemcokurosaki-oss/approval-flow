...
async function loadProgress() {
    ...
    const { data: allReqs } = await db
        .from('approval_requests')
        .select('id, project_number, machine_name, flow_type, status, has_inspection, test_run, created_at, updated_at, confirmed_shipping_date, tentative_shipping_date, tentative_shipping_confirmed_at, inspection_date, inspection_time, requester_id, sheet_data, approval_steps(approver_id, status)')
    ...
    const { data: machineTasks } = await db.from('tasks')
        ...
        .in('text', ['機械組立', '外観検査', '試運転', '出荷確認会議', '出荷準備', '工場出荷'])
        .not('machine', 'is', null);
