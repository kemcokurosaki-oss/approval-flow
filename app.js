    ...
    const html = nums.map(num => {
        ...
        return `<div class="prog-card">
            <div class="prog-card-header">
                <span class="prog-card-num">${esc(num)}</span>${label ? `<span class="prog-card-label">${esc(label)}</span>` : ''}${packingDateLabel}${shippingDateLabel}
            </div>
            ${machineRows}
        </div>`;
    }).join('');
...
async function openDetailModal(requestId) {
    ...
    document.getElementById('detail_body').innerHTML = `
        ...
        ${subInfoParts.length ? `<div style="font-size:14px;color:#888;margin-top:4px;display:flex;flex-wrap:wrap;column-gap:16px;row-gap:2px;">${subInfoParts.map(p => `<span style="white-space:nowrap;">${p}</span>`).join('')}</div>` : ''}
