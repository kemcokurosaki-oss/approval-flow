...
async function loadMineSide() {
    ...
    el.innerHTML = reqs.map(req => {
        ...
        <div class="side-card" onclick="openDetailModal('${req.id}')">
            ...
            ${pInfo.customer_name ? `<div class="side-card-sub">${esc(pInfo.customer_name)}</div>` : ''}
            ${pInfo.project_details ? `<div class="side-card-sub">${esc(pInfo.project_details)}</div>` : ''}
...
async function loadProgress() {
    ...
    const { data: allReqs } = await db
        .from('approval_requests')
        .select('id, project_number, machine_name, flow_type, status, has_inspection, test_run, updated_at, confirmed_shipping_date, inspection_date, inspection_time, approval_steps(approver_id, status)')
...
async function onSiMachineChange() {
    ...
    `<div style="padding:2px 0; color:#1e3a5f; font-weight:bold; margin-top:4px;">▶　簡易検査開催案内（今回）</div>`;
