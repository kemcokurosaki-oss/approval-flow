...
function canApplyFlow(flowType) {
    ...
    if (flowType === 'simple_inspection' || flowType === 'inspection' ||
        flowType === 'shipping_meeting'  || flowType === 'shipping'  ||
        flowType === 'shipping_prep')  return isQorS;
    return false;
}
...
// ===== フロー6: 出荷準備完了確認 =====
function openShippingPrepModal() {
    currentSpProjectNum = '';
    document.getElementById('sp_project_display').textContent = '';
    document.getElementById('sp_project_info').style.display  = 'none';
    document.getElementById('sp_machine_group').style.display = 'none';
    document.getElementById('sp_machine_list').innerHTML      = '';
    document.getElementById('sp_flow_box').style.display      = 'none';
    document.getElementById('sp_note_input').value             = '';

    document.getElementById('shipping_prep_modal').classList.add('open');
}

function closeShippingPrepModal() {
    document.getElementById('shipping_prep_modal').classList.remove('open');
}

async function onSpProjectChange() {
    const num = currentSpProjectNum;
    document.getElementById('sp_project_info').style.display  = 'none';
    document.getElementById('sp_machine_group').style.display = 'none';
    document.getElementById('sp_flow_box').style.display      = 'none';
    if (!num) return;
    const p = projectsMap[num] || {};
    document.getElementById('sp_customer_display').textContent = p.customer_name || '—';
    document.getElementById('sp_project_name_display').textContent = p.project_details || '—';
    document.getElementById('sp_project_info').style.display = 'block';
    showLoading('読み込み中...');
    try {
        await _loadMachineCheckboxes(num, 'sp_machine_list', 'onSpMachineChange');
        document.getElementById('sp_machine_group').style.display = 'block';
    } finally {
        hideLoading();
    }
}

async function onSpMachineChange() {
    const num      = currentSpProjectNum;
    const machines = getSelectedMachines('sp_machine_list');
    if (machines.length === 0) { document.getElementById('sp_flow_box').style.display = 'none'; return; }
    const machine = machines[0];
    showLoading('読み込み中...');
    let doneFlows, required;
    try {
        doneFlows = await _getMachineDoneFlows(num, machine);
        required  = await _getRequiredFlows(num, machine);
    } finally {
        hideLoading();
    }
    const rows = [...required].filter(t => t !== 'shipping_prep').map(t => ({ type: t, label: FLOW_LABELS[t] || t }));
    document.getElementById('sp_flow_list').innerHTML = rows.map(f => `<div class="flow-info-item">
        <span class="flow-info-icon">${doneFlows.has(f.type) ? '✅' : '──'}</span>
        <span class="${doneFlows.has(f.type) ? 'flow-info-done' : 'flow-info-upcoming'}">${esc(f.label)}</span>
        ${doneFlows.has(f.type) ? '<span class="flow-info-note">承認済み</span>' : ''}
    </div>`).join('') + `<div class="flow-info-item" style="margin-top:6px;"><span class="flow-info-current">▶ 出荷準備完了確認（今回）</span></div>`;
    document.getElementById('sp_flow_box').style.display = 'block';
}

async function submitShippingPrep() {
    const num      = currentSpProjectNum;
    const machines = getSelectedMachines('sp_machine_list');
    const note     = document.getElementById('sp_note_input').value.trim();

    if (!num)                  { showToast('工事番号が設定されていません', 'error'); return; }
    if (machines.length === 0) { showToast('機械を選択してください', 'error'); return; }

    const btn = document.getElementById('sp_submit_btn');
    ...
    try {
        for (const machine of machines) {
            const { error } = await db.from('approval_requests').insert({
                project_number: num, machine_name: machine, flow_type: 'shipping_prep', status: 'submitted',
                requester_id: currentUser.id, note: note || null
            });
            if (error) throw error;
        }
        closeShippingPrepModal();
        await refreshAll();
        showToast(`出荷準備完了確認を起票しました。（${machines.length}機械）`, 'success');
    } catch (e) {
        showToast('起票に失敗しました: ' + e.message, 'error');
    } finally {
        btn.disabled = false; btn.textContent = '起票する';
        hideLoading();
    }
}
