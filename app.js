...
function renderProgressCards() {
    ...
    const { baseNums, projectData, machineTaskSet, projectFlowSet, shippingApproverNameMap, taskInfoMap, projectFlowInfoMap, shippingTasksMap, unitListMap, unitTaskInfoMap } = progressCachedData;
    const hasTask        = (num, machine, taskText) => machineTaskSet.has(`${num}__${machine}__${taskText}`);
        ...
        const machineRows = machines.map(machine => {
            ...
            const nodes = applicable.map((f, i) => {
                ...
                return `<div class="flow-node${clickable}" ${clickAttr}
                    ...
                    <div class="flow-label"><span style="position:relative; display:inline-block;">${esc(isAssemblyGroup ? '組立・電装' : f.label)}${isAssemblyGroup ? '<span style="position:absolute; left:100%; top:50%; transform:translateY(-50%);">⚡</span>' : ''}</span></div>
