        ...
        groupsHtml.push(`
            <div class="settings-flow-group">
                ...
                `).join('') : (g.kind === 'department'
                    ? '<div class="settings-note">該当者がいません。<button class="btn btn-xs btn-secondary" onclick="showRecipientMasterScreen()">「宛先候補の管理」で追加する →</button></div>'
...
async function submitShippingMeeting() {
    ...
    try {
        for (const machine of machines) {
            ...
            const roomEmail = flowSettings.roomEmails[location];
            if (roomEmail) {
                await db.from('approval_notifications').insert({
